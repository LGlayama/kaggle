...
F5,T5= stacking1(model,X_train,y,X_test)

NF = np.concatenate(( F1,F2,F3,F4,F5), axis=1)
NT = np.concatenate((T1,T2,T3,T4,T5), axis=1)
...
#                silent=1)
#xgb_param = x.get_xgb_params()
#xgb_param['num_class'] = 2


#search1 = grid_search.GridSearchCV(
...
#rf = RandomForestClassifier(n_estimators=180,max_depth=15)
# random_forest.fit(X_train,y)
# y_p=random_forest.predict_proba(X_test)
...
# solution.to_csv("xgb_f.csv", index = False)
